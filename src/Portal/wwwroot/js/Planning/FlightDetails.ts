	...
	export class LastItem {
		...
		export class CityDetail {
			...
			public createLayout($lastBox) {
				...
				this.slider = new RangeSlider(this.$layout.find(".days-range"), "daysRange");
				this.slider.genSlider(1, 21);
				this.slider.onRangeChanged = () => {
					this.genMonthFlights();
				}

				this.monthsSel = new MonthsSelector(this.$layout.find(".months"));
			...
			private genMonthFlights() {		
					...
					this.v.apiGet("SkypickerCity", prms, (fs) => {
							...
							$ofCont.show();
							this.$layout.find(".other-flights-filter").show();
	...
	export class FlightDetails {
		...
		public genFlights($cont, flights) {
			...
			lg.customMapping = (i) => {
				return {
					price: i.Price,
					scoreText: this.getScoreText(i.FlightScore),
					scoreStars: AnytimeAggUtils.getScoreStars(i.FlightScore)
				};
			};
